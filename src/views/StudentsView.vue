...
import {studentsStatuses} from '@/assets/statuses'
import {studentsHeaders} from '@/assets/headers'
import {studentsFilters} from '@/assets/filters'
import {studentsCreateFields} from '@/assets/entityFields'
import {studentsUpdateFields} from '@/assets/entityFields'
...
export default {
    ...
    methods: {
        ...
        actionCreate() {
            this.entityModalTitle = 'Создание Ученика' 
            this.entityModalAction = 'Создать'
            this.entityModalFields = studentsCreateFields
        },
        actionEdit(entity) {
            this.selectedEntity = entity
            this.entity = this.entityToString(entity)
            this.entityModalTitle = 'Изменение Ученика' 
            this.entityModalAction = 'Изменить'
            this.entityModalFields = studentsUpdateFields
        },
        actionDelete(entity) {
            this.selectedEntity = entity
        ...
        actionAccepted() {
            EntityAPI.delete(this.selectedEntity.id).then(() => {
                ...
                M.toast({html: "Ученик удален", classes: 'green-text white'})
                }).catch((e) => {
                ...
                if (id) {
                    if (JSON.stringify(this.selectedEntity) !== JSON.stringify(entity)) {
                        ...
                        EntityAPI.put(id, entity).then(() => {
                            ...
                            M.toast({html: "Ученик изменен!", classes: 'green-text white'})
                            }).catch((e) => {
                                M.toast({html: 'Ошибка! ' + e.response.data.message, classes: 'red-text white'})
                                })
                        this.entity = ''
                    ...
                    EntityAPI.post(entity).then(() => {
                        this.fetchData()
                        M.toast({html: "Ученик создан!", classes: 'green-text white'})
                        }).catch((e) => {
                            M.toast({html: 'Ошибка! ' + e.response.data.message, classes: 'red-text white'})
                            })
                    this.entity = ''
