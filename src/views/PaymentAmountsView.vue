...
import {paymentAmountsHeaders} from '@/assets/headers'
import {paymentAmountsFilters} from '@/assets/filters'
import {paymentAmountsCreateFields} from '@/assets/entityFields'
import {paymentAmountsUpdateFields} from '@/assets/entityFields'

export default {
    components: { EntityHeader, EntityTable, EntityModal, ModalConfirm, PreLoader },
    name: "PaymentAmountsView",
    data: () => ({
        isLoad: false,
        ...
        headers: paymentAmountsHeaders,
        filters: paymentAmountsFilters,
        data: []
    }),
    methods: {
        async fetchData(queryFilter) {
            ...
            EntityAPI.get(queryFilter).then((res) => {
                res.data.forEach((row, index) => {
                    ...
                    res.data[index]['sum'] = row['sum'].toString()
                })
                this.data = res.data
                }).catch((e) => {
        ...
        actionCreate() {
            this.entityModalTitle = 'Создание Стоимости' 
            this.entityModalAction = 'Создать'
            this.entityModalFields = paymentAmountsCreateFields
        },
        actionEdit(entity) {
            this.selectedEntity = entity
            this.entity = this.entityToString(entity)
            this.entityModalTitle = 'Изменение Стоимости' 
            this.entityModalAction = 'Изменить'
            this.entityModalFields = paymentAmountsUpdateFields
        },
        actionDelete(entity) {
            this.selectedEntity = entity
        ...
        actionAccepted() {
            EntityAPI.delete(this.selectedEntity.id).then(() => {
                ...
                M.toast({html: "Стоимость удалена", classes: 'green-text white'})
                }).catch((e) => {
                ...
                if (id) {
                    if (JSON.stringify(this.selectedEntity) !== JSON.stringify(entity)) {
                        ...
                        EntityAPI.put(id, entity).then(() => {
                            ...
                            M.toast({html: "Стоимость изменена!", classes: 'green-text white'})
                            }).catch((e) => {
                                M.toast({html: 'Ошибка! ' + e.response.data.message, classes: 'red-text white'})
                                })
                    }
                    ...
                    EntityAPI.post(entity).then(() => {
                        this.fetchData()
                        M.toast({html: "Стоимость создана!", classes: 'green-text white'})
                        }).catch((e) => {
                            M.toast({html: 'Ошибка! ' + e.response.data.message, classes: 'red-text white'})
                            })
                    this.entity = ''
